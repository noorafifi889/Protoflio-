...
<div className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
  ...
  {/* vignette: لايت أبيض ناعم + الدارك زي ما هو */}
  <div className="absolute inset-0
  ...
  <FloatingIcons />
      ...
      <Container>
          <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.35 }} className="grid items-center gap-10 md:grid-cols-2 md:gap-14">
            ...
            <motion.p
              variants={item}
              className="mt-5 max-w-xl text-sm leading-7 text-black/50  dark:text-white/50  md:text-base"
            >
...
function Stat({ value, label }: { value: string; label: string }) {
  return (
    <div>
      ...
      <div className="mt-1 text-xs text-black/55 dark:text-white/50">{label}</div>
    </div>
  );
}